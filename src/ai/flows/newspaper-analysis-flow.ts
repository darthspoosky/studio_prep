...
// Utility to validate MCQ formatting and metadata
export function validateMcqs(text: string): { valid: boolean; errors: string[] } {
  const mcqs = text.match(/<mcq[^>]*>.*?<\/mcq>/gs) || [];
  const errors: string[] = [];

  mcqs.forEach((mcq, index) => {
    const prefix = `MCQ ${index + 1}:`;

    if (!/difficultyScore="\d+"/.test(mcq)) {
      errors.push(`${prefix} missing difficultyScore`);
    }

    const optionTags = mcq.match(/<option[^>]*>/g) || [];
    if (optionTags.length !== 4) {
      errors.push(`${prefix} should contain 4 <option> tags`);
    }

    const correctCount = optionTags.filter(opt => /correct="true"/.test(opt)).length;
    if (correctCount !== 1) {
      errors.push(`${prefix} must have exactly one correct option`);
    }
  });
